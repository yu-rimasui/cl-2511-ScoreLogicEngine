...
【分析の絶対ルール：数値的根拠（エビデンス）の提示】
**「なんとなく後半が悪かった」といった定性的な表現は禁止します。**
必ず、「後半の平均スコアが前半より+4.5打悪化している」「Par5での平均スコアが6.8打と、Par4（4.9打）に比べて苦戦している」といった**具体的な数値**を用いて論拠を示してください。数字こそが信頼の証です。
...
ただし、D1-D155の番号自体はレポートに記載しないでください。
${LOGIC_DEFINITIONS}
...
## 📊 総合サマリー
(今回のラウンドを一言で表すキャッチフレーズと、全体的な総評を3-4行で。スコア、パット数、FWキープ率などの主要スタッツも織り交ぜること)
...
- [分析コメント] **「上がり3ホールの平均」や「前半/後半のスコア差」を数値で示すこと。**

### 3. 良い流れ・悪い流れの連続性
(バーディ後の油断、ボギー後の立て直し、連続ミスの傾向など。D56-D57, D77-D81, D102等を活用)
- [分析コメント] **「連続パーの回数」や「ダボ直後のホールの平均スコア」などに言及すること。**

### 4. スコアに隠れた「真の原因」
(ユーザーが気づいていない、パット数やFWキープ率の裏にある根本的な課題や強み。D14-D15, D48, D62等を活用)
- [分析コメント] **表面的なスコアだけでなく、パット数や隠れた指標を数値で示すこと。**
...
---
**※過去データとの比較**
過去データが存在する場合、**「前回比でパット数が-3改善」「直近3ラウンドの平均スコアより+5打悪い」**といった具体的な比較数値を必ず盛り込んでください。成長の軌跡を数字で可視化します。
`;